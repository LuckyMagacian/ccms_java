...
//		logger.info("生成mybatis插入语句->类名:"+clazz.getName()+"->表名:"+tableName);
		...
		temp.append("<insert id=\"add"+sName.replace("Bean","")+"\" "+
					"parameterType=\""+tName+"\">\n");
...
//		logger.info("生成mybatis删除语句->类名:"+clazz.getName()+"->表名:"+tableName);
		...
		String tName=clazz.getName();		//类全名
		String sName=clazz.getSimpleName();	//类名
		temp.append("<delete id=\"delete"+sName.replace("Bean","")+"\" "+
					"parameterType=\""+tName+"\">\n");
		...
		for(Field each: fields){
			...
			for(int i=0;i<chars.length;i++){
				...
				temp.append("<if test=\""+each.getName()+" != null\"> and "+name+" = #{"+each.getName()+"}  </if>  \n");
		}
		temp.append("</where>\n</delete>\n");
		return temp.toString();
	}
	/**
	 * 根据 类对象和表名 创建 update sql语句 
	 * where部分请根据需要手动删除!!!
	 * 若仅保留一个条件 请把<if>标签也去掉,否则将会更新所有记录
	 * @param clazz
	 * @param tableName
	 * @return
	 */
...
//		logger.info("生成mybatis更新语句->类名:"+clazz.getName()+"->表名:"+tableName);
		...
		String sName=clazz.getSimpleName();	//类名
		List<Field> list=new ArrayList<Field>();
		temp.append("<update id=\"update"+sName.replace("Bean","")+"\" "+
					"parameterType=\""+tName+"\">\n");
		...
		for(Field each: fields){
			...
			temp.append("<if test=\""+each.getName()+" != null\"> "+name+" = #{"+each.getName()+"},</if>  \n");
		}
		temp.append("</set>\n<where>\n");
		for(Field each: list){
			each.setAccessible(true);
...
//		logger.info("生成mybatis查询语句->类名:"+clazz.getName()+"->表名:"+tableName);
		...
		String sName=clazz.getSimpleName();	//类名
		List<Field> list=new ArrayList<Field>();
		temp.append("<select id=\"select"+sName.replace("Bean","")+"\" "+
				"resultMap=\""+sName.replace("Bean","").replace(sName.charAt(0),(char)(sName.charAt(0)+32))+"Map\" "+
...
//		logger.info("生成mybatis ResiltMap语句->类名:"+clazz.getName());
		...
		String tName=clazz.getName();		//类全名
		String sName=clazz.getSimpleName();	//类名
		temp.append("<resultMap id=\""+sName.replace("Bean","").replace(sName.charAt(0),(char)(sName.charAt(0)+32))+"Map\" "+ 
					"type=\""+tName+"\">\n");
		Field[] fields=clazz.getDeclaredFields();
		for(Field each: fields){
			each.setAccessible(true);
